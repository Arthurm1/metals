...
final case class MetalsPath(
  ...
  override def relativize(other: Path): MetalsPath = {
    ...
    // both being !absolute is valid but return value is undocumented so throw exception as Metals doesn't use it
    if (!isAbsolute())
  ...
  def isJDK: Boolean =
    names.lengthCompare(3) >= 0 && names(1) == MetalsFileSystem.jdkSubName
  ...
  def jarPath: Option[MetalsPath] = {
    if (names.lengthCompare(3) < 0) None else Some(subpath(0, 3))
  }
  ...
  // metalsfs:/xxx/name.jar/package/class -> Some("name.jar")
  def jarName: Option[String] =
    if (!isAbsolute || names.lengthCompare(3) < 0) None else Some(names(2))

  // metalsfs:/xxx/name.jar/package/class -> Seq("package", "class")
  def jarDirs: Seq[String] =
    if (!isAbsolute || names.lengthCompare(3) < 0) Seq.empty
    else names.toSeq.drop(3)

  // metalsfs:/xxx/name.jar/package/class -> package/class
  def classPath: Option[MetalsPath] =
    if (!isAbsolute || names.lengthCompare(3) < 0) None
    else {
      val classPathNames = new Array[String](names.length - 3)
      var i = 0
      while (i < classPathNames.length) {
        classPathNames(i) = names(i + 3)
        i = i + 1
      }
      Some(MetalsPath(metalsFileSystem, classPathNames))
    }

  // metalsfs:/xxx/name.jar/package/class -> Some("module-info.java") or Some("module-info.class") or None
  def moduleName: Option[String] =
    if (!isAbsolute || names.lengthCompare(3) < 0) None
    else {
      // jdk and source jars use module/module-info.java
      ...
      if (Files.exists(moduleInfoLocation))
        Some(moduleLocationNames(3))
      else {
        // TODO I think non-source jars use module/module-info.class
        moduleLocationNames(4) = "module-info.class"
        if (Files.exists(moduleInfoLocation))
          Some(moduleLocationNames(3))
        else
          None
...
object MetalsPath {
  // readonly/metalsLibraries/jdk/temerin17/XXX -> metalsfs://metalsLibraries/jdk/temerin17/XXX
